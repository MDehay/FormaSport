...
INSERT INTO `formation` VALUES (1,'ss','http://192.168.1.50/ppe4_final/model/img/mini/téléchargement.png','image',11,'Janvier','Mars',11),(2,'ss','http://192.168.1.50/ppe4_final/model/img/mini/téléchargement.png','image',11,'Janvier','Mars',11),(3,'ss','http://192.168.1.50/ppe4_final/model/img/mini/téléchargement.png','image',11,'Janvier','Mars',11),(4,'ss','http://192.168.1.50/ppe4_final/model/img/mini/téléchargement.png','image',11,'Janvier','Mars',11),(5,'sd','http://192.168.1.50/ppe4_final/model/img/mini/fate.jpeg','image',11,'Janvier','Janvier',11);
...
INSERT INTO `lieu` VALUES (1,'Valenciennes - Salle de Sport'),(2,'Lille - Salle Valour'),(3,'Valencienne - Salle des fêtes '),(4,'Valencienne - Salle des fêtes ');
...
--
-- Temporary table structure for view `reponsevote_reunion`
--

DROP TABLE IF EXISTS `reponsevote_reunion`;
/*!50001 DROP VIEW IF EXISTS `reponsevote_reunion`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `reponsevote_reunion` AS SELECT 
 1 AS `NumeroReunion`,
 1 AS `Organisateur`,
 1 AS `Object`,
 1 AS `Descripition`,
...
INSERT INTO `reunion_valide` VALUES (10,9,10),(11,9,10),(12,3,1),(13,11,12),(14,12,13);
...
--
-- Final view structure for view `reponsevote_reunion`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `reponsevote_reunion` AS select `disponible`.`FK_RE` AS `NumeroReunion`,`r`.`RE_Organisateur` AS `Organisateur`,`r`.`RE_Object` AS `Object`,`r`.`RE_Description` AS `Descripition`,`l`.`LI_Libelle` AS `Lieu`,`r`.`RE_Fin_Valide` AS `FinValidite`,count(`disponible`.`DIS_Choix`) AS `TotalVote`,count((case `disponible`.`DIS_Choix` when 0 then 1 else NULL end)) AS `PasVote`,count((case `disponible`.`DIS_Choix` when 1 then 1 else NULL end)) AS `Disponible`,count((case `disponible`.`DIS_Choix` when 2 then 1 else NULL end)) AS `PasDisponible` from ((`disponible` join `reunion` `r` on((`disponible`.`FK_RE` = `r`.`ID_RE`))) join `lieu` `l` on((`r`.`FK_LI` = `l`.`ID_LI`))) group by `disponible`.`FK_RE` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2019-05-04 16:38:16
